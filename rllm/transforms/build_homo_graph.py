...
def build_homo_graph(
    ...
    Args:
        df (pd.DataFrame): The given dataframe, where the first two columns represent
            two types of nodes that may be connected in a homogeneous graph 
            (abbreviated as sorce nodes and target nodes).
            Assume that the indices of source and target nodes in the dataframe start from 0.
        ...
        names (List[str]): The names of the two types of nodes in the generated graph.
        transform (Optional[Callable]): A function/transform that takes in a :obj:`GraphData`
