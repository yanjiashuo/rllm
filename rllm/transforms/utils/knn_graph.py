...
def knn_graph(
    ...
    Args:
        ...
        mode (str[`connectivity`, `distance`], optional): Type of returned matrix: 
            `connectivity` will return the connectivity matrix with ones and zeros, 
            while `distance` will return the distances between neighbors according to the given metric.
            (default: `connectivity`)
        metric (str[`minkowski`, `cosine`, `l1`, `l2`, ...], optional): Metric to use for distance computation. 
            Default is `minkowski`, which results in the standard Euclidean distance when p = 2.
            (default: `minkowski`)
        metric_paramsdict (dict, optinal): Additional keyword arguments for the metric function.
            (default: None)
        include_self (bool, optinal): If set to True, the graph will contain self-loops. (default: False)
