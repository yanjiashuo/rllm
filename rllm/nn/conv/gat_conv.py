...
class GATConv(torch.nn.Module):
    ...
    to handle graph-structured data, similiar to <https://github.com/Diego999/pyGAT>.
    ...
    .. math::
        ...
        \mathbf{\Theta} \mathbf{x}_i+ \mathbf{a}^{\top} \mathbf{\Theta}\mathbf{x}_j
        \right)\right)}
    ...
    Args:
        ...
        concat (bool): If set to `False`, the multi-head attentions are averaged instead
            of concatenated.
        negative_slop (float): LeakyReLU angle of the negative slope, the default value is 0.2.
    ...
    def __init__(self,
        ...
        if isinstance(in_channels, int):
            self.fc_src = self.fc_tgt = torch.nn.Linear(in_channels, heads * out_channels, bias=False)
        else:
            self.fc_src = torch.nn.Linear(in_channels[0], heads * out_channels, bias=False)
            self.fc_tgt = torch.nn.Linear(in_channels[1], heads * out_channels, bias=False)
    ...
    def forward(self, inputs: Union[Tensor, Tuple[Tensor, Tensor]], adj: Tensor):
        ...
        edge_h = torch.cat((h_src[edge[0, :], :, :], h_tgt[edge[1, :], :, :]), dim=2)
        ...
        for i in range(self.heads):
            h_prime = torch_sparse.spmm(edge_t, edge_e[:, i], M, N, h_src[:, i, :])
            assert not torch.isnan(edge_e).any()
            e_rowsum = torch_sparse.spmm(edge_t, edge_e[:, i], M, N, torch.ones(size=(N, 1), device=device))
