...
class SAGEConv(torch.nn.Module):
    ...
    Args:
        ...
        aggr_methods (str): The aggregation method to use, *e.g.*, `mean`, `max_pooling`
            `mean_pooling`, `gcn`, `lstm`.
        act: (Callable): The activation function is applied to aggreagtion, 
            the default function is ReLU.
        concat (bool): If set to `False`, the multi-head attentions are averaged instead
            of concatenated.
    ...
    def __init__(self, 
        ...
        if aggr_method == 'mean':
            self.aggr_module = MeanAggregator(in_channels, out_channels, act, dropout, dropout, bias)
        elif aggr_method == 'max_pooling':
            self.aggr_module = MaxPoolingAggregator(in_channels, in_channels, out_channels, act, concat, dropout, bias)
        elif aggr_method == 'mean_pooling':
            self.aggr_module = MeanPoolingAggregator(in_channels, in_channels, out_channels, act, concat, dropout, bias)
        elif aggr_method == 'gcn':
            self.aggr_module = GCNAggregator(in_channels, out_channels, act, dropout, bias)
        elif aggr_method == 'lstm':
            self.aggr_module = LSTMAggregator(in_channels, in_channels, out_channels, act, concat, dropout, bias)
        else:
...
class Aggregator(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        self.self_weight = Parameter(torch.empty(self_channels, out_channels))
        self.neigh_weight = Parameter(torch.empty(neigh_channels, out_channels))
    ...
    def __repr__(self) -> str:
        return (f'{self.__class__.__name__}({self.self_channels}, {self.neigh_channels}, '
                f'{self.out_channels}')
...
class MeanAggregator(Aggregator):
    ...
    def __repr__(self) -> str:
        return (f'{self.__class__.__name__}({self.self_channels}, '
                f'{self.out_channels}')
...
class GCNAggregator(torch.nn.Module):
    ...
    def forward(self, self_vecs, neigh_vecs):
        ...
        means = torch.mean(torch.cat([self_vecs.unsqueeze(1), neigh_vecs], dim=1), dim=1)
...
class LSTMAggregator(Aggregator):
    ...
    def __init__(self, 
        ...
        self.rnn = torch.nn.LSTM(in_channels, hidden_channels, batch_first=True)
