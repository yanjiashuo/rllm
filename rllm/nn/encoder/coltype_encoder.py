...
class ColTypeTransform(Module, ABC):
    ...
    specific columns type into 3-dimensional column-wise tensor that is input into
    tabular deep learning models. Columns with same ColType will be transformed into
    tensors.
    ...
    def __init__(
        ...
        r"""Since many attributes are specified later, this is a fake initialization"""
        super().__init__()
    ...
    def na_forward(self, feat: Tensor) -> Tensor:
        ...
        for col in range(feat.size(1)):
            if self.na_mode == NAMode.MOST_FREQUENT:
                ...
                # fill_value = torch.mean(feat[:, col][~torch.isnan(feat[:, col])])
...
class CategoricalTransform(ColTypeTransform):
    ...
    def __init__(
        ...
        super().__init__(out_channels, stats_list, col_type, post_module, na_mode)
