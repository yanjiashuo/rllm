...
class TabTransformer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        shared_category_dim_divisor: int = 8.,   # in paper, they reserve dimension / 8 for category shared embedding
        col_stats_dict: Dict[ColType, List[Dict[str, Any]]] = None,
    ):
        super().__init__()
        if layers <= 0:
            raise ValueError(f"num_layers must be a positive integer (got {layers})")
        ...
        self.transformer = nn.ModuleList([
            TabTransformerConv(hidden_dim, heads, head_dim, attn_dropout, ff_dropout)
            for _ in range(layers)
        ])

        # 3. Prepare decoder
        input_size = (hidden_dim * self.categorical_col_len) + self.numerical_col_len if ColType.NUMERICAL in self.col_stats_dict.keys() else (hidden_dim * self.categorical_col_len)
    ...
    def forward(self, feat_dict: Dict[ColType, Tensor], return_attns: bool=False):
        ...
        if ColType.CATEGORICAL in self.col_stats_dict.keys():
            ...
            if self.use_shared_categ_embedding:
                shared_category_embedding = \
                    self.shared_category_embedding.unsqueeze(0).expand(category_embedding.shape[0], -1, -1)
                category_embedding = torch.cat((category_embedding, shared_category_embedding), dim = -1)
        # 2. Get representations table convs
            for layer in self.transformer:
                category_embedding, post_softmax_attn = layer(category_embedding, return_attn=True)
    ...
    def _prepare_column_transform(self):
        if ColType.CATEGORICAL in self.col_stats_dict.keys():
            ...
            shared_embedding_dim = 0 if not self.use_shared_categ_embedding else \
                int(self.hidden_dim // self.shared_category_dim_divisor)
            ...
            if self.use_shared_categ_embedding:
                self.shared_category_embedding = nn.Parameter(torch.zeros(self.categorical_col_len, shared_embedding_dim))
