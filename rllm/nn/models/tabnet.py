...
def create_emb_group_matrix(group_matrix, cat_idxs, input_dim, cat_emb_dim, post_embed_dim):
    ...
    embedding_group_matrix = torch.empty((n_groups, post_embed_dim),
...
class TabNetEncoder(torch.nn.Module):
    r"""Defines main part of the TabNet network without the embedding layers."""
    def __init__(
        self,
        input_dim,
        output_dim,
...
class TabNet(torch.nn.Module):
    ...
    Args:
        ...
        n_a (int): intDimension of the attention  layer (usually between 4 and 64).
        n_steps (int): Number of successive steps in the network (usually between 3 and 10).
        gamma (int): Float above 1, scaling factor for attention updates (usually between 1.0 to 2.0).
        cat_emb_dim Union ([int, List[int]]): Size of the embedding of categorical features.
        ...
        mask_type (str): Either "sparsemax" or "entmax" : this is the masking function to use
        group_attention_matrix (Tensor): Matrix of size (n_groups, input_dim), 
    ...
    def __init__(
        ...
        self.input_dim = len(self.col_stats_dict[ColType.CATEGORICAL]) if ColType.NUMERICAL not in self.col_stats_dict.keys()\
            else len(self.col_stats_dict[ColType.CATEGORICAL]) + len(self.col_stats_dict[ColType.NUMERICAL])
        if self.n_steps <= 0:
        ...
        group_attention_matrix = create_group_matrix(self.grouped_features, self.input_dim)
        self.post_embed_dim = self.input_dim - len(self.col_stats_dict[ColType.CATEGORICAL]) + \
            len(self.col_stats_dict[ColType.CATEGORICAL]) * cat_emb_dim
        emb_group_matrix = create_emb_group_matrix(group_attention_matrix, 
                                                   self.cat_idxs, 
                                                   self.input_dim, 
                                                   self.cat_emb_dim, 
                                                   self.post_embed_dim,
    ...
    def forward(self, feat_dict):
        ...
        if ColType.CATEGORICAL in self.col_stats_dict.keys():
            ...
            flatten_category = category_embedding.reshape(category_embedding.size(0), -1)
            xs.append(flatten_category)

        if ColType.NUMERICAL in self.col_stats_dict.keys():
            x_numeric = feat_dict[ColType.NUMERICAL]
            numerical_embedding = self.numeric_transform(x_numeric)
            flatten_numeric = numerical_embedding.reshape(numerical_embedding.size(0), -1)
...
class AttentiveTransformer(torch.nn.Module):
    ...
    Args:
        ...
        virtual_batch_size (int): Batch size for Ghost Batch Normalization.
        momentum (float): Float value between 0 and 1 which will be used for momentum in batch norm.
        mask_type (str): Either "sparsemax" or "entmax" : this is the masking function to use.
        """
    def __init__(
        self,
