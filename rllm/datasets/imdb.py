...
class IMDB(Dataset):
    ...
    as collected in the `MAGNN: Metapath Aggregated Graph Neural Network for Heterogeneous Graph
    Embedding <https://arxiv.org/abs/2002.01680>`__ paper.
    ...
    def process(self):
        ...
        data.train_mask = index2mask(torch.from_numpy(split['train_idx']).to(torch.long), len(y))
        data.val_mask = index2mask(torch.from_numpy(split['val_idx']).to(torch.long), len(y))
        data.test_mask = index2mask(torch.from_numpy(split['test_idx']).to(torch.long), len(y))
