...
class TAPEDataset(Dataset):
    ...
    urls = {
        'original text': {
            'cora': 'https://drive.usercontent.google.com/download?id=1hxE0OPR7VLEHesr48WisynuoNMhXJbpl&confirm=t',
            'pubmed': 'https://drive.usercontent.google.com/download?id=1sYZX-jP6H8OkopVa9cp8-KXdEti5ki_W&confirm=t'
        },
        'LLM responses': {
            'cora': 'https://drive.usercontent.google.com/download?id=1tSepgcztiNNth4kkSR-jyGkNnN7QDYax&confirm=t',
            'pubmed': 'https://drive.usercontent.google.com/download?id=166waPAjUwu7EWEvMJ0heflfp0-4EvrZS&confirm=t'
        },
        'GPT predictions': {
            'cora': 'https://raw.githubusercontent.com/XiaoxinHe/TAPE/main/gpt_preds/cora.csv',
            'pubmed': 'https://raw.githubusercontent.com/XiaoxinHe/TAPE/main/gpt_preds/pubmed.csv'
        }
    }
    ...
    def download(self):
        ...
        path_text = download_url(self.urls['original text'][self.name], self.raw_dir, f'{self.name}_orig.zip')
        path_llm = download_url(self.urls['LLM responses'][self.name], self.raw_dir, f'{self.name}.zip')
        path_preds = download_url(self.urls['GPT predictions'][self.name], self.raw_dir, f'{self.name}.csv')
    ...
    def _get_raw_text_pubmed(self):
        ...
        # parse graph
        with open(osp.join(path, 'Pubmed-Diabetes.DIRECTED.cites.tab'), 'r') as edge_file:
            # first two lines are headers
            edge_file.readline()
            edge_file.readline()
