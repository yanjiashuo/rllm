...
class NeighborSampler(torch.utils.data.DataLoader):
    ...
    Args:
        adj (Tensor): an adjacency matrix which defines the underlying graph
            connectivity flow.
        num_samples (List): The number of neighbors to sample for each node in each
            layer. If set to `num_samples[l] = -1`, all neighbors are included in layer l.
    ...
    def sample(self, batch):
        ...
        neighs_samples = neighs_samples[0] if len(neighs_samples) == 1 else neighs_samples[::-1]
