...
def add_remaining_self_loops(adj: Tensor, fill_value: Any = 1.):
    ...
    if is_torch_sparse_tensor(adj):  
        ...
        fill_values = torch.ones_like(loop_index, dtype=values.dtype, device=device) * fill_value
...
def construct_graph(edge_index: Tensor, N: int, edge_attr: Tensor = None, remove_self: bool = True):
...
def gcn_norm(adj: Tensor):
    ...
    Classification with Graph Convolutional Networks" <https://arxiv.org/abs/1609.02907>`__ .
    ...
    if is_torch_sparse_tensor(adj):
        ...
        adj_sl_sp = sp.csr_matrix((adj_sl_data, (indices_sl[0], indices_sl[1])), shape=shape)
