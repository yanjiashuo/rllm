...
from rllm.llm.prompt.default_prompt import DEFAULT_SCENARIO_CLASSIFICATION_TMPL, DEFAULT_SCENARIO_REGRESSION_TMPL
...
class Predictor:
    ...
    def __init__(
        ...
        if prompt is None:
            assert type in ['classification', 'regression'], "type must be 'classification' or 'regresssion'!"
            function_mapping = {'sample_description': generate_sample_description}
            if type == 'classification':
                self.prompt=PromptTemplate(DEFAULT_SCENARIO_CLASSIFICATION_TMPL, function_mappings=function_mapping)
            else:
                self.prompt=PromptTemplate(DEFAULT_SCENARIO_REGRESSION_TMPL, function_mappings=function_mapping)
        else:
            self.prompt=prompt
