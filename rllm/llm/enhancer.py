...
from rllm.llm.prompt.utils import generate_sample_description, get_template_vars
...
class Enhancer:
    ...
    Args:
        ...
        type (Optional[Literal['explanation|embedding', 'explanation', 'embedding']] ):
    ...
    .. code-block:: python
        ...
        outputs = enhancer(data, cols=['text', 'explanation']) # Embedding columns 'text' and 'explanation'
    """
    def __init__(
        self,
        ...
        llm: LLM = None,
        llm_embed: LLM = None,
        type: Optional[Literal['explanation|embedding', 'explanation', 'embedding']] = 'explanation|embedding',
    ) -> None:
        # NOTE: Only support `PromptTemplate` so far, only support `explanation` so far!
        if 'explanation' in type:
            self._llm = llm
        if 'embedding' in type:
            self._llm_embed = llm_embed

        assert type in ['explanation|embedding', 'explanation', 'embedding'], "type error!"
        self.type = type

        if 'explanation' in self.type:
            from rllm.llm.prompt.base import PromptTemplate
            if prompt is None:
                function_mapping = {'sample_description': generate_sample_description}
                self.prompt = PromptTemplate(DEFAULT_SCENARIO_EXPLANATION_TMPL, function_mappings=function_mapping)
            else:
                self.prompt = prompt
        ...
        **kwargs,
        ...
        if 'explanation' in self.type:
            ...
            for var in required_variables:
                assert var in input_variables, f"Variable '{var}' not found in input variables."
        ...
        if 'embedding' in self.type:
            if 'explanation' in self.type:
                ...
                cols = kwargs['cols'] if 'cols' in kwargs else ['text'] # default target column is 'text'.
                inputs = [col.values.tolist() for col_name, col in df[cols].items()]
